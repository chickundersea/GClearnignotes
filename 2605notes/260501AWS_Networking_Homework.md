...
Endpoint C : 用Lambda 因為只需呼叫外部 API 回傳結果，無狀態常駐伺服器。且放 VPC 外可直接連公網，免 NAT 費用
Endpoint E: EC2 E 需要存取 CloudWatch Logs API，但不應經過公網。PrivateLink 讓流量走 AWS 內部網路就好

- 高可用與擴展: 如何確保SLA？若使用量上升是否可擴充？
目前因為EC2放在單一AZ
會需要調整分散不同 AZ + Auto Scaling Group來根據請求擴充
- 資安防禦: 如何防止惡意流量？最小權限原則？
- 目前 I/O bound 都設定最小範圍的流量
- 可以再把 WAF 掛在 ALB 前面，防 SQL injection、XSS、DDoS等攻擊

- 成本預估: $
  ┌──────────────────────────┬─────────────────────────┬────────────────────┐
  │ 項目                     │ 單價                    │ 月費用             │
  ├──────────────────────────┼─────────────────────────┼────────────────────┤
  │ ALB                      │ $16.2（固定）+ LCU 費用 │ ~$18               │
  ├──────────────────────────┼─────────────────────────┼────────────────────┤
  │ EC2 t3.micro × 4         │ $0.0136/hr × 4 × 730hr  │ ~$40               │
  ├──────────────────────────┼─────────────────────────┼────────────────────┤
  │ NAT Gateway              │ $0.062/hr + $0.062/GB   │ ~$45（固定）+ 流量 │
  ├──────────────────────────┼─────────────────────────┼────────────────────┤
  │ VPC Endpoint (Interface) │ $0.014/hr × 730hr       │ ~$10               │
  ├──────────────────────────┼─────────────────────────┼────────────────────┤
  │ Lambda C                 │ 100 萬次免費額度內      │ ~$0                │
  ├──────────────────────────┼─────────────────────────┼────────────────────┤
  │ 資料傳輸                 │ 低流量                  │ ~$2                │
  └──────────────────────────┴─────────────────────────┴────────────────────┘
